...
class EGNNDynamics(nn.Module):
    def __init__(self, phar_nf, residue_nf,
        ...
        self.phar_encoder = nn.Sequential(
            nn.Linear(phar_nf, 2 * phar_nf),
            act_fn,
            nn.Linear(2 * phar_nf, joint_nf)
        )

        self.phar_decoder = nn.Sequential(
            nn.Linear(joint_nf, 2 * phar_nf),
            act_fn,
            nn.Linear(2 * phar_nf, phar_nf)
        )
    ...
    def forward(self, xh_phars, xh_residues, t, mask_phars, mask_residues):

        x_phars = xh_phars[:, :self.n_dims].clone()
        h_phars = xh_phars[:, self.n_dims:].clone()

        x_residues = xh_residues[:, :self.n_dims].clone()
        h_residues = xh_residues[:, self.n_dims:].clone()

        # embed phar features and residue features in a shared space
        h_phars = self.phar_encoder(h_phars)
        h_residues = self.residue_encoder(h_residues)

        # combine the two node types
        x = torch.cat((x_phars, x_residues), dim=0)
        h = torch.cat((h_phars, h_residues), dim=0)
        mask = torch.cat([mask_phars, mask_residues])
        ...
        if self.mode == 'egnn_dynamics':
            update_coords_mask = None if self.update_pocket_coords \
                else torch.cat((torch.ones_like(mask_phars),
        ...
        # decode phar and residue features
        h_final_phars = self.phar_decoder(h_final[:len(mask_phars)])
        h_final_residues = self.residue_decoder(h_final[len(mask_phars):])
        ...
        return torch.cat([vel[:len(mask_phars)], h_final_phars], dim=-1), \
               torch.cat([vel[len(mask_phars):], h_final_residues], dim=-1)
