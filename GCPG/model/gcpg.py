...
from model.graphtransformer_layers import DGLGraphTransformerBlock
from model.egat_layers import EGATEncoderBlock
...
class GCPG(nn.Module):
    ...
    def __init__(self, params, tokenizer):
        ...
        wrong_params = set(params.keys()) - GCPG.PARAM_SET
        ...
        self.cond_embedding = nn.Sequential(
            nn.Linear(params['cond_dim'], hidden_dim),
            nn.PReLU(),
        ...
        self.pp_encoder = EGATEncoderBlock(hidden_dim, hidden_dim, n_layers=params['pp_encoder_n_layer'],
                                           num_heads=8, dropout=0, batch_norm=True)
        ...
        self.word_embed = nn.Embedding(vocab_size, hidden_dim)  #在输入端加入性质
        ...
        self.expand = nn.Sequential(
                nn.Linear(hidden_dim, hidden_dim),             # 也许在这里加就可以了
                nn.ReLU(),
                nn.LayerNorm(hidden_dim),
                nn.Linear(hidden_dim, hidden_dim),
    ...
    def calculate_z(self, inputs, input_mask, vvs, pp_mask, cond_embedd, cond_mask):
        ...
        cond_with_ppxt = torch.cat((cond_embedd, ppxt), dim=0)

        cond_encoder_full_mask = torch.cat((cond_mask, encoder_full_mask), dim=1)


        cond_ppxt = self.encoder(cond_with_ppxt, cond_encoder_full_mask)  # (s b f), input masks need not transpose
        #xxt = cond_ppxt[NUM_CONDITIONS + MAX_NUM_PP_GRAPHS:, :, :]
        xxt = cond_ppxt[1 + MAX_NUM_PP_GRAPHS:, :, :]

        foo = xxt.new_ones(1, *xxt.shape[1:])
        z, _ = self.attention(foo, xxt, xxt, key_padding_mask=input_mask)  # query, key, value   # return attn, attn_weights
        z = z.squeeze(0)
    ...
    def process_p(self, pp_graphs):
        ...
        v = self.pp_encoder(pp_graphs, v, e)
    ...
    def expand_then_fusing(self, z, pp_mask, vvs, cond_mask, cond_embedd):
        zz = self.expand(z)  # seq batch feat  这里的Z也要加入cond
        ...
        cond_full_mask = torch.cat((cond_mask, full_mask), dim=1)

        zzz = torch.cat((vvs, zzs), dim=0)  # seq_plus batch feat
        czzz = torch.cat((cond_embedd, zzz), dim=0)
        #if not self.remove_dencoder:
        czzz = self.dencoder(czzz, cond_full_mask)

        return czzz, cond_full_mask

    @torch.jit.unused
    def forward(self, inputs, input_mask, pp_graphs, targets, conditions):
        vv, vvs, pp_mask = self.process_p(pp_graphs)

        cond_init = torch.cat([*conditions], dim=0).t()  # dim需要修改
        cond_embedd = self.cond_embedding(cond_init).unsqueeze(0)
        cond_mask = torch.ones(cond_embedd.size(1), dtype=torch.bool, device=cond_init.device).unsqueeze(1)

        # 需要在calculate_z 里面加入性质去计算Z
        z, kl_loss = self.calculate_z(inputs, input_mask, vvs, pp_mask, cond_embedd, cond_mask)
        # 同时在Z上面expand一下特征
        czzz, cond_full_mask = self.expand_then_fusing(z, pp_mask, vvs, cond_mask, cond_embedd)
        ...
        # predict
        output = self.decoder(target_embed, czzz,
                              x_mask=target_mask, mem_padding_mask=cond_full_mask).permute(1, 0, 2).contiguous()
    ...
    def ag_forward(self, inputs, input_mask, pp_graphs, conditions, random_sample=False):
        vv, vvs, pp_mask = self.process_p(pp_graphs)
        ...
        czzz, cond_encoder_mask = self.expand_then_fusing(z, pp_mask, vvs,cond_mask, cond_embedd)

        predict, scores = self._generate(czzz, cond_encoder_mask,random_sample=random_sample, return_score=True)

        return predict, scores, kl_loss

    @torch.jit.export
    @torch.no_grad()
    def generate(self, pp_graphs, conditions, random_sample=False, return_z=False):
        vv, vvs, pp_mask = self.process_p(pp_graphs)
        ...
        zzz, encoder_mask = self.expand_then_fusing(z, pp_mask, vvs, cond_mask, cond_embedd)
