...
def train_fn(train_loader, model, optimizer, epoch, scheduler, beta=1, scaler=None):
    ...
    for step, batch_data in tqdm(enumerate(train_loader), disable=disable_tqdm, total=N):
        ...
        gate = torch.tensor([1, 1, 1, 1, 1, 0, 0], dtype=torch.float32)
        ...
        if CFG.fp16:
            with amp.autocast():
                prediction_scores, mapping_scores, lm_loss, kl_loss = model(inputs, input_mask, pp_graphs, targets, selected_conditions)
        else:
            prediction_scores, mapping_scores, lm_loss, kl_loss = model(inputs, input_mask, pp_graphs, targets, selected_conditions)
...
if __name__ == '__main__':
    ...
    train_dataset = SemiSmilesDataset(train_smiles, train_MW, train_logP, train_qed, train_SAS,
                                      train_HBA, train_HBD, train_RotaNumBonds, train_Score,
                                      train_Strain, tokenizer,
                                      use_random_input_smiles, use_random_target_smiles)
    valid_dataset = SemiSmilesDataset(valid_smiles, valid_MW, valid_logP, valid_qed, valid_SAS,
                                      valid_HBA, valid_HBD, valid_RotaNumBonds, valid_Score,
                                      valid_Strain, tokenizer,
                                      use_random_input_smiles, use_random_target_smiles)
