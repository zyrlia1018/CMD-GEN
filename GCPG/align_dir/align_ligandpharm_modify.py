...
def align_ligand_to_pharmacophore_with_tolerance(smiles, pharmacophoric_points, tolerance, num_gen, output_dir, name):
    ...
    # Create SDF folder
    sdf_folder = os.path.join(output_dir, name)
            ...
            for j, smiles in enumerate(smiles_list):
                try:
                    align_ligand_to_pharmacophore_with_tolerance(smiles, pharm_group, args.phar_tolerance,
                                                             args.num_gen, args.output_dir, name=f"lig{j}_phar{i}")
