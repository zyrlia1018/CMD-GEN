...
PP_TYPE_WEIGHT = [1.4891304347826086, 1.0, 8.058823529411764, 1.0378787878787878, 1.8026315789473686, 2.174603174603175,
                  17.125]
# PP_TYPE_WEIGHT = [2.751600120186175, 1.4710733531754452, 12.484900709949583, 17.1907274916767, 1.0, 1.600326420257839, 3.8980283499979924,
#                  15.71160818334844]
...
MODEL_DEFAULT_SETTINGS = {
    ...
    "cond_dim": 7,  # number of condition
...
class CFG:
    ...
    init_epoch = 0  # 16
    reload_path = './checkpoints/final_model.pth'
    reload_ignore = []
...
def train_fn(train_loader, model, optimizer, epoch, scheduler, beta=1, scaler=None):
    ...
    for step, batch_data in tqdm(enumerate(train_loader), disable=disable_tqdm, total=N):
        ...
        #这里创建一个门控张量，0 表示关闭条件，1 表示打开条件
        gate = torch.tensor([0, 0, 0, 0, 0, 1, 0], dtype=torch.float32)

        # 使用门控条件
        selected_conditions = [gate[i] * conditions[i] for i in range(len(conditions))]
...
if __name__ == '__main__':
    ...
    with open('/home/data/zou/CMD-GEN/GCPG/gcpg_gate/data_prepare/chembl33_filtered_train_data.pickle', 'rb') as f:
    ...
    with open('/home/data/zou/CMD-GEN/GCPG/gcpg_gate/data_prepare/chembl33_filtered_valid_data.pickle', 'rb') as f:
    ...
    with open('/home/data/zou/CMD-GEN/GCPG/gcpg_gate/data_prepare/chembl33_filtered_test_data.pickle', 'rb') as f:
    ...
    with open('/home/data/zou/CMD-GEN/GCPG/gcpg_gate/data_prepare/docking_atm_epoch1_properties.pickle', 'rb') as f:
        train_properties = pickle.load(f)
        train1_smiles = train_properties["smiles"][1000:]
        train1_MW = train_properties["MW"][1000:]
        train1_logP = train_properties["logP"][1000:]
        train1_qed = train_properties["qed"][1000:]
        train1_SAS = train_properties["SAS"][1000:]
        train1_HBA = train_properties["HBA"][1000:]
        train1_HBD = train_properties["HBD"][1000:]
        train1_RotaNumBonds = train_properties["RotaNumBonds"][1000:]
        train1_Score = train_properties["score"][1000:]
        train1_Smi = train_properties["smi"][1000:]
        valid1_smiles = train_properties["smiles"][:1000]
        valid1_MW = train_properties["MW"][:1000]
        valid1_logP = train_properties["logP"][:1000]
        valid1_qed = train_properties["qed"][:1000]
        valid1_SAS = train_properties["SAS"][:1000]
        valid1_HBA = train_properties["HBA"][:1000]
        valid1_HBD = train_properties["HBD"][:1000]
        valid1_RotaNumBonds = train_properties["RotaNumBonds"][:1000]
        valid1_Score = train_properties["score"][:1000]
        valid1_Smi = train_properties["smi"][:1000]


    all_smiles = set(train_smiles + valid_smiles + test_smiles)

    # gen_smiles = list(np.random.RandomState(CFG.seed).
    #                   choice(valid1_smiles, CFG.gen_size, replace=False)) * CFG.gen_repeat
    ...
    print(use_random_target_smiles)
    train_dataset = SemiSmilesDataset(train1_smiles, train1_MW, train1_logP, train1_qed, train1_SAS,
                                      train1_HBA, train1_HBD, train1_RotaNumBonds, train1_Score,
                                      train1_Smi, tokenizer,
                                      use_random_input_smiles, use_random_target_smiles)
    valid_dataset = SemiSmilesDataset(valid1_smiles, valid1_MW, valid1_logP, valid1_qed, valid1_SAS,
                                      valid1_HBA, valid1_HBD, valid1_RotaNumBonds, valid1_Score,
                                      valid1_Smi, tokenizer,
                                      use_random_input_smiles, use_random_target_smiles)
