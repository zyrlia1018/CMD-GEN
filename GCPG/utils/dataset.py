...
class SemiSmilesDataset(Dataset):
    ...
    def __init__(self, smiles_list, mw_list, logp_list, qed_list, sa_list, hba_list, hbd_list,
                 RotaNumBonds_list, score_list, smi_list, tokenizer: Tokenizer,
        ...
        self.hba_list = hba_list
        self.hbd_list = hbd_list
        self.RotaNumBonds_list = RotaNumBonds_list
    ...
    def __getitem__(self, item):
        MW = self.mw_list[item]
        logP = self.logp_list[item]
        QED = self.qed_list[item]
        SAS = self.sa_list[item]
        HBA = self.hba_list[item]
        HBD = self.hbd_list[item]
        RotaNumBonds = self.RotaNumBonds_list[item]
        Score = self.score_list[item]
        Smi = self.smi_list[item]
        ...
        return corrupted_input, pp_graph, mapping_, target_seq, MW, logP, QED, SAS, HBA, HBD, RotaNumBonds, Score, Smi
    @staticmethod
    def collate_fn(batch):
        pad_token = Tokenizer.SPECIAL_TOKENS.index('<pad>')

        corrupted_inputs, pp_graphs, mappings, target_seqs, MW, logP, QED, SAS, \
        HBA, HBD, RotaNumBonds, Score, Smi, *other_descriptors = list(zip(*batch))
        ...
        mappings = pad_sequence(mappings, batch_first=True,
                                padding_value=-100)  # torch cross entropy loss ignores -100 by default, but we do not use cross_entropy_loss acctually

        target_seqs = pad_sequence(target_seqs, batch_first=True, padding_value=pad_token)

        return corrupted_inputs, input_mask, pp_graphs, mappings, target_seqs, torch.tensor(MW).unsqueeze(0),\
               torch.tensor(logP).unsqueeze(0), torch.tensor(QED).unsqueeze(0), torch.tensor(SAS).unsqueeze(0), \
               torch.tensor(HBA).unsqueeze(0), torch.tensor(HBD).unsqueeze(0), torch.tensor(RotaNumBonds).unsqueeze(0),\
               torch.tensor(Score).unsqueeze(0), torch.tensor(Smi).unsqueeze(0)
